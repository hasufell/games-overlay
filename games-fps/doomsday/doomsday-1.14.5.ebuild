...
inherit python-r1 confutils eutils qt4-r2
...
src_prepare() {
	...
	echo "PREFIX=/usr" >> config_user.pri
	...
	if use !dedicated ; then
		...
		if use doom; then
			...
			if use resources; then
				res_arg="-def /usr/share/${PN}/defs/jdoom/jDRP.ded"
			fi
		...
		if use heretic; then
			...
			elog "/usr/share/${PN}/data/jheretic"
			elog
		fi
	fi
}
